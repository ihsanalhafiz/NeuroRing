...
#pragma HLS bind_storage variable = head type = ram_2p impl = uram
...
#pragma HLS PIPELINE II = 4
      ...
      if (SynForward_right.read_nb(pkt)) {
        have_right = true;
        if (pkt == 0)
          done_right = true;
      } else if (SynForward_left.read_nb(pkt)) {
        have_left = true;
        if (pkt == 0)
          done_left = true;
      }
...
#pragma HLS PIPELINE II = 1
          float_to_uint32 temp_conv;
          Delay_t delay = pkt.range(7 + (64 * i), 0 + (64 * i));
          DstID_t dst = pkt.range(31 + (64 * i), 8 + (64 * i));
          temp_conv.u = pkt.range(63 + (64 * i), 32 + (64 * i));
