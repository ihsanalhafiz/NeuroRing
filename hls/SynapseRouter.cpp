...
void Router(
    ...
    uint32_t                     CoreID,
    hls::stream<synapse_list_t> &SynForwardOut0,
                ...
                if (pkt.data[31] == 1) {
                    ...
                    if(pkt.data[30] == 1){
                        ...
                        if((dest/8) == CoreID || (dest2/8) == CoreID){
                            ...
                            if(lane0 == 0 || lane1 == 0) SynForwardOut0.write(pkt.data);
                            if(lane0 == 1 || lane1 == 1) SynForwardOut1.write(pkt.data);
                            if(lane0 == 2 || lane1 == 2) SynForwardOut2.write(pkt.data);
                            if(lane0 == 3 || lane1 == 3) SynForwardOut3.write(pkt.data);
                            if(lane0 == 4 || lane1 == 4) SynForwardOut4.write(pkt.data);
                            if(lane0 == 5 || lane1 == 5) SynForwardOut5.write(pkt.data);
                            if(lane0 == 6 || lane1 == 6) SynForwardOut6.write(pkt.data);
                            if(lane0 == 7 || lane1 == 7) SynForwardOut7.write(pkt.data);                        
                        }
                    }
                }
            }
...
void Accumulator(
    hls::stream<synapse_list_t> &SynForward,
    ...
    uint32_t                     NeuronStart)
{
    ...
    time_loop: for (int t = 0; t < SimulationTime; t++) {
        t_accumulator = t;
        // Accumulation phase - read synapses until end marker
        bool done = false;
        accumulate_loop: while (!done) {
            #pragma HLS PIPELINE II=4
            
            synapse_list_t pkt_new;
            SynForward.read(pkt_new);
            if(pkt_new == 0) {
                done = true;
            }
            else {
                for(int i = 0; i < 4; i++) {
                    ...
                    if(dst.to_uint() >= NeuronStart && dst.to_uint() < NeuronStart + (NEURON_NUM/8)) {
                        ...
                        conv_u.f = new_f;
                        if(upper) word.range(63, 32) = conv_u.u;
                        else      word.range(31, 0) = conv_u.u;
        ...
        output_loop: for(int i = 0; i < NEURON_NUM/16; i++) {
...
void weightforward(
    hls::stream<stream_weight_t> &SpikeWeight0,
...
{
    // Interface pragmas
    #pragma HLS INTERFACE axis port=synapse_stream
    #pragma HLS INTERFACE axis port=SpikeOutWeight
    #pragma HLS INTERFACE axis port=syn_route_in
    #pragma HLS INTERFACE axis port=syn_forward_rt
    ...
    Accumulator5: Accumulator(
        SynForwardOut5,
        SpikeWeight5,
        SimulationTime,
        NeuronStart + ((NEURON_NUM/8)*5)
    );
    Accumulator6: Accumulator(
        SynForwardOut6,
        SpikeWeight6,
        SimulationTime,
        NeuronStart + ((NEURON_NUM/8)*6)
    );
    Accumulator7: Accumulator(
        SynForwardOut7,
        SpikeWeight7,
        SimulationTime,
