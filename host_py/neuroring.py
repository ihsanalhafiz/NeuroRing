                ...
                if neuron_total > 0 and neuron_start >= 0 and group_sources.shape[0] > 0:
                    ...
                    for src_gid, first_idx_for_src, cnt, pidx, vm_bits, dc_bits in zip(
                        ...
                        if cnt > 0:
                            ...
                            # Pivoted sort around neuron_start:
                            # First: tgts >= neuron_start (ascending)
                            # Then:  tgts < neuron_start (ascending)
                            ge_mask = tgts >= np.uint32(neuron_start)
                            idx_ge = np.nonzero(ge_mask)[0]
                            idx_lt = np.nonzero(~ge_mask)[0]
                            order_ge = idx_ge[np.argsort(tgts[idx_ge], kind='stable')]
                            order_lt = idx_lt[np.argsort(tgts[idx_lt], kind='stable')]
                            order = np.concatenate((order_ge, order_lt), axis=0)
